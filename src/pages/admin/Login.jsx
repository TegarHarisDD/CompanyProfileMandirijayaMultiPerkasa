...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (lockoutUntil && Date.now() < lockoutUntil) {
      ...
      setErrorMsg(`Terlalu banyak percobaan. Coba lagi dalam ${remaining} detik.`);
      return;
    }
    ...
    if (error) {
      ...
      if (newCount >= 5) {
        const lockoutMs = Date.now() + 60 * 1000;
        setLockoutUntil(lockoutMs);
        setErrorMsg('Terlalu banyak percobaan. Akun dikunci selama 60 detik.');
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 px-6">
      <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md border border-slate-800">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          >
            {loading ? 'Memproses...' : (lockoutUntil && Date.now() < lockoutUntil ? 'Terkunci' : 'Masuk Dashboard')}
            {!loading && !(lockoutUntil && Date.now() < lockoutUntil) && <ArrowRight size={18} />}
          </button>
        </form>
      </div>
    </div>
