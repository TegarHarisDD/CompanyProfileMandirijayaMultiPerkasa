...
const ALLOWED_MAP_DOMAINS = ['google.com', 'google.co.id', 'maps.google.com', 'maps.googleapis.com'];

function isValidMapUrl(url) {
  try {
    const hostname = new URL(url).hostname;
    return ALLOWED_MAP_DOMAINS.some(domain => hostname.endsWith(domain));
  } catch {
    return false;
  }
}
...
export default function Contact() {
  ...
  const safeMapUrl = isValidMapUrl(companyInfo?.map_url)
    ? companyInfo.map_url
    : "https://maps.google.com/maps?q=JL.+Gondomono+Raya+Semarang&t=&z=15&ie=UTF8&iwloc=&output=embed";
